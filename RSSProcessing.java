...
public final class RSSProcessing {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
        ...
        String titleLabel = item.child(titleIndex).child(0).label();
        out.println("Title: " + titleLabel);

        int linkIndex = getChildElement(item, "link");
        String linkLabel = item.child(linkIndex).child(0).label();
        out.println("Link: " + linkLabel);

        int descIndex = getChildElement(item, "description");
        String descLabel = item.child(descIndex).child(0).label();
        out.println("Description: " + descLabel);
    ...
    public static void main(String[] args) {
        ...
        String titleLabel = channel.child(titleIndex).child(0).label();
        out.println("Title: " + titleLabel);

        int linkIndex = getChildElement(channel, "link");
        String linkLabel = channel.child(linkIndex).child(0).label();
        out.println("Link: " + linkLabel);

        int descIndex = getChildElement(channel, "description");
        String descLabel = channel.child(descIndex).child(0).label();
        out.println("Description: " + descLabel);
